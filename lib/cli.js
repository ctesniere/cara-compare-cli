	...
	return Promise.map(dataFiles, function (data) {
		...
		_.forEach(objectMerge, function (value, key) {
			...
			_.forEach(value, function (data, fileId) {
				valuesOnlyDiff.push((data.value === undefined || data.value === '') ? data.defaultValue : data.value);
		...
		console.log(numberTokenPrint + ' Token affiché sur ' + _.size(objectMerge));
	});
});
